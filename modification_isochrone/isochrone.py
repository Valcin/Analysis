...
if not on_rtd:
	...
	from .interp import interp_value, interp_values, interp_eep, interp_eeps, bolcorr
...
class Isoeep(object):
	...
	def __call__(self, age, feh, distance=10, AV=0.0, x_ext=None, ext_table=False):
		if x_ext is not None:
			if x_ext==0.:
				ext = extcurve_0
			else:
		...
		# ~print(AAV)
		dm = 5*np.log10(distance) - 5 
		#~ mag, tefff, loggg, debut = self.ic.interp_eep(age, feh, self.icol)
		mag, debut = self.ic.interp_eep(age, feh, self.icol)
		#~ print(np.min(mag), np.max(mag))
		#~ print(dm, E, dm + E)
		#~ bc = self.ic.bolcorr(self.icol, tefff, loggg, feh, AV)
		return mag + dm + E, debut
		#~ return mag + dm #+ Alamb
...
class FastIsochrone(Isochrone):
	...
	def _make_grid(self, recalc=False):
		...
		if os.path.exists(self._npz_filename) and not recalc:
			...
			sfehs = (self.fehs[np.where(self.fehs > -3.)[0]])
			nsfehs = len(sfehs)
			df_list = [[self.df.loc[f,a] for f in sfehs] for a in self.ages]
			lens = np.array([[len(df_list[i][j]) for j in range(nsfehs)]
							 for i in range(self.Nage)]).T #just because
			data = np.zeros((nsfehs, self.Nage, lens.max(), self.Ncols))

			for i in range(self.Nage):
				for j in range(nsfehs):
					N = lens[j,i]
	...
	def interp_value(self, mass, age, feh, icol): # 4 is log_g
		...
		#~ print(self._grid[6,101,81,:])
		if self._ages is None:
			self._initialize()

		try:
			return interp_value(float(mass), float(age), float(feh), icol,
								self.grid,self.eep_col, self.mass_col,
	...
	def interp_eep(self, age, feh, icol): # 4 is log_g
		#~ print(np.shape(self._grid))
		#~ print(self._grid[6,101,81,:])
		if self._ages is None:
			self._initialize()
		sfehs = (self.fehs[np.where(self.fehs > -3.)[0]])
		#~ print(self.fehs)
		if np.max(self.ages) == 10.30:
			return interp_eep(float(age), float(feh), icol,
									self.grid,self.eep_col, self.mass_col,
									self.ages, self.fehs, self.grid_Ns, self.debug)
		else:
			return interp_eep(float(age), float(feh), icol,
								self.grid,self.eep_col, self.mass_col,
								self.ages, sfehs, self.grid_Ns, self.debug)



	def bolcorr(self, icol, tefff, loggg, feh, AV): # 4 is log_g
		#~ print(np.shape(self._grid))
		#~ print(self._grid[6,101,81,:])

		return bolcorr(icol, tefff, loggg, feh, AV)
